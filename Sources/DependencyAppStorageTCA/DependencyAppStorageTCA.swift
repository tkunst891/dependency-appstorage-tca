...
extension Dependency.AppStorage.Values {
	...
	/// and forwards this change to a reducer´s `State`.
	///
	...
	public func bind<Action>(to keyPath: WritableKeyPath<Action.State, Value>) -> Effect<Action>
	where Action: BindableAction, Action.State: ObservableState, Value: Equatable {
		return .run { send in
...
extension BindingReducer {
	...
	/// - Parameters:
	///   - state: Keypath to the state's member whose value should be copied to AppStorage.
	...
	/// - Returns: A reducer which first executes the base reducer and then issues a `.run` Effect which will asynchronously set the `@Dependency.AppStorage` value.
	public func toAppStorage<Settings: DependencySettings, Value>(
		state: KeyPath<Self.State, Value>,
...
where Parent.Action: BindableAction, Parent.State == Parent.Action.State {
	let parent: Parent
	let fromState: KeyPath<State, Value>
	...
	public func reduce(into state: inout Parent.State, action: Parent.Action) -> Effect<Parent.Action> {
		...
		if let bindingAction = (/Action.binding).extract(from: action), bindingAction.keyPath == fromState {
			result = result.concatenate(
				with: Settings.shared.set(toAppStorage, to: state[keyPath: fromState])
			)
		}
		return result
	}
...
extension _AppStorageReducer {
	...
	///   - appStorage: Keypath to a member of a class conforming to the ``DependencySettings`` protocol. This member will be set to the same value as the state.
	/// - Returns: A reducer which first executes the base reducer and then issues a `.run` Effect which will asynchronously set the `@Dependency.AppStorage` value.
	public func toAppStorage<NextValue>(
		state: KeyPath<Self.State, NextValue>,
